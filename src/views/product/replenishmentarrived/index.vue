...
import {reqApproveReplenishment, reqReplenishment } from "@/api/product/replenishment"
...
const getHasReplenishment = async () => {
  let result: replenishmentResponseData = await reqReplenishment(pageNo.value, limit.value, 'replenishing', 'all');
  if (result.code === 1) {
...
const updateReplenishment = async (row) => {
  let result: any = await reqApproveReplenishment(row.id, row.productId, row.quantity);
  if (result.code === 1) {
