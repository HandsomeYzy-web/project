      ...
      <el-table style="margin: 10px 0px" border :data="replenishmentArr">
        ...
        <el-table-column label="操作">
          <template #default="{ row }">
            ...
            <el-button type="danger" size="small" icon="Close" @click="cancelReplenishment(row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
const getHasReplenishment = async () => {
  let result: replenishmentResponseData = await reqReplenishment(pageNo.value, limit.value, 'pending', 'all');
  if (result.code === 1) {
...
const updateReplenishment = async (row) => {
  let result: any = await reqUpdateReplenishment(row.id, 'approved', userStore.userId);
  if (result.code === 1) {
...
const cancelReplenishment = async (row) => {
  let result: any = await reqUpdateReplenishment(row.id, 'rejected', userStore.userId);
  if (result.code === 1) {
